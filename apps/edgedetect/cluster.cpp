...
void cluster(char *image_gray, char *temp_buf, char *filter, char *output) {
  // XRT kernel interface: one AXI master port per pointer argument for
...
#pragma HLS bind_storage variable = local_temp_buf type = RAM_2P impl = BRAM
  for (int paramIdx_1 = 0, localIdx_1 = 0;
       paramIdx_1 < 2073600 && localIdx_1 < 1036800;
...
#pragma clava param = output type = PRIMITIVE_POINTER in = LIVEIN out =        \
    LIVEOUT_USEDLATER size = 2073600
#pragma clava param = filter type = PRIMITIVE_POINTER in = LIVEIN out =        \
    LIVEOUT size = 9
#pragma clava param = temp_buf type = PRIMITIVE_POINTER in = LIVEIN out =      \
    LIVEOUT size = 2073600
#pragma clava param = image_gray type = PRIMITIVE_POINTER in = LIVEIN out =    \
    LIVEOUT size = 2073600
...
#pragma HLS loop_tripcount max = 3
          sum_i0 =
              sum_i0 + local_image_gray[(r_i0 + i_i0) * 1920 + (c_i0 + j_i0)] *
                           local_filter[i_i0 * 3 + j_i0];
        }
      }
      output[(r_i0 + dead_rows_i0) * 1920 + (c_i0 + dead_cols_i0)] =
          (sum_i0 / normal_factor_i0);
    }
  }
...
#pragma HLS loop_tripcount max = 3
          sum_i2 = sum_i2 + output[(r_i2 + i_i2) * 1920 + (c_i2 + j_i2)] *
                                local_filter[i_i2 * 3 + j_i2];
        }
      }
      local_image_gray[(r_i2 + dead_rows_i2) * 1920 + (c_i2 + dead_cols_i2)] =
          (sum_i2 / normal_factor_i2);
    }
  }
...
#pragma HLS loop_tripcount max = 3
          sum_i4 = sum_i4 + output[(r_i4 + i_i4) * 1920 + (c_i4 + j_i4)] *
                                local_filter[i_i4 * 3 + j_i4];
        }
      }
