...
void llama2_loop(
    ...
    int *rtr_val,
    // synthcalls
    int8_t *printf_tokens_buf,
    async_kernel_info *printf_tokens_info,
    int8_t *fflush_buf,
...
void generate(Transformer *transformer, Tokenizer *tokenizer, Sampler *sampler, char *prompt, int steps)
{
    ...
    async_call *printf_tokens = create_async_call_variadic(PRINTF, 256, tokenizer_max_token_length);
    async_call *fflush_stdout = create_async_call_fixed(FFLUSH, 256);
    async_call *printf_newline = create_async_call_fixed(PRINTF, 1);
    ...
    {
        listening = false;
        listening |= listen_async_printf(printf_tokens, "%c");
        listening |= listen_async_fflush(fflush_stdout, stdout);
        listening |= listen_async_printf(printf_newline, "\n");
    } while (listening);
